...
// dependency and durable-control-plane diagnostics.
...
export async function GET(request: NextRequest) {
    ...
    try {
      ...
      const [
        ...
        supabase
          .from('reports')
          .select(
            'outcome_followup_claimed_at,is_retrospective_assignment,valuation_effective_date,valuation_effective_date_source'
          )
          .limit(1),
        supabase
          .from('property_data')
          .select(
            'unit_count,unit_count_source_type,unit_count_source_reference'
          )
          .limit(1),
        supabase
          .from('county_rules')
      ...
      const schemaErrors = [
        reportSchema.error
          ? `reports: ${reportSchema.error.message}`
          : null,
        propertySchema.error
          ? `property_data: ${propertySchema.error.message}`
          : null,
        jurisdictionSchema.error
          ? `county_rules: ${jurisdictionSchema.error.message}`
          : null,
      ].filter((message): message is string => Boolean(message));

      results.supabase_schema =
        schemaErrors.length > 0
          ? {
              status: 'error',
              message:
                'Required delivery and report-workfile schema is missing or inaccessible: ' +
                schemaErrors.join('; '),
            }
          : {
              status: 'ok',
              message:
                'Delivery, follow-up, valuation-date provenance, evidence provenance, and jurisdiction-plugin schema is current',
            };
        ...
        results.supabase_storage =
          missingBuckets.length > 0
            ...
            : {
                status: 'ok',
                message:
                  'Storage accessible (required buckets present)',
              };
      }
  ...
  try {
    const queue = await getPipelineQueueHealth();
    const overdueSeconds = queue.oldest_due_age_seconds ?? 0;
    const queueStalled =
      queue.expired_lease_count > 0 || overdueSeconds > 15 * 60;

    results.pipeline_queue = {
      status: queueStalled ? 'error' : 'ok',
      message: [
        `${queue.queued_count} queued`,
        `${queue.running_count} running`,
        `${queue.retrying_count} retrying`,
        `${queue.dead_letter_count} dead-letter`,
        `${queue.expired_lease_count} expired leases`,
        queue.oldest_due_age_seconds === null
          ? 'no overdue work'
          : `oldest due ${queue.oldest_due_age_seconds}s`,
      ].join('; '),
    };
  } catch (error) {
    results.pipeline_queue = {
      status: 'error',
      message: `Durable pipeline control plane unavailable: ${
        error instanceof Error ? error.message : String(error)
      }`,
    };
  }
  ...
  results.ai_appraiser = openAiConfigured
    ? {
        status: 'ok',
        message: `OpenAI / ${AI_MODELS.PRIMARY}`,
      }
    : {
        status: 'not_configured',
  ...
  results.ai_vision = openAiConfigured
    ? {
        status: 'ok',
        message: `OpenAI / ${AI_MODELS.VISION}`,
      }
    : {
        status: 'not_configured',
  ...
  results.ai_document = openAiConfigured
    ? {
        status: 'ok',
        message: `OpenAI / ${AI_MODELS.DOCUMENT}`,
      }
    : {
        status: 'not_configured',
  ...
  results.fast_ai = fastAi.configured
    ? {
        status: 'ok',
        message: `${fastAi.provider} / ${fastAi.model}`,
      }
    : {
        status: 'not_configured',
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    ...
    results.stripe = {
      status: 'ok',
      message: `Key configured (${isTest ? 'TEST' : 'LIVE'} mode)`,
    };
  }

  results.stripe_publishable =
  ...
  results.resend = process.env.RESEND_API_KEY
    ? {
        ...
        message: `Configured. From: ${
          process.env.RESEND_FROM_ADDRESS ?? 'not set'
        }`,
      }
    : {
        status: 'not_configured',
  ...
  results.google_maps =
    ...
    process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY
      ? {
          status: 'ok',
          message:
            'Primary geocoding, address search, static maps, and Street View configured',
        }
      : {
          status: 'not_configured',
          message:
            'Google Maps key missing — Census and transitional map fallbacks will be used',
        };
  results.attom = process.env.ATTOM_API_KEY
    ? {
        status: 'ok',
        message:
          'Structured property enrichment configured',
      }
    : {
        status: 'not_configured',
        message:
          'ATTOM_API_KEY missing — only lower-confidence public-record fallbacks are available',
      };
  results.serper = process.env.SERPER_API_KEY
    ? {
        status: 'ok',
        message:
          'Current-source web retrieval configured',
      }
    : {
        status: 'not_configured',
        message:
          'SERPER_API_KEY missing — current-source research is disabled',
      };
  results.azure_maps_fallback =
    process.env.AZURE_MAPS_SUBSCRIPTION_KEY
      ? {
          status: 'ok',
          message:
            'Transitional map fallback configured',
        }
      : {
          status: 'not_configured',
          message:
            'Azure Maps fallback not configured',
        };
  results.mapillary_fallback =
    process.env.NEXT_PUBLIC_MAPILLARY_ACCESS_TOKEN
      ? {
          status: 'ok',
          message:
            'Fallback street imagery configured',
        }
      : {
          status: 'not_configured',
          message:
            'Mapillary fallback not configured',
        };
