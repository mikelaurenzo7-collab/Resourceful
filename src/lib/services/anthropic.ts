...
Use this parcel data to write a substantive site description with actual dimensions, shape, and zoning analysis. For HBU, reference actual zoning restrictions. This is independently-sourced data that strengthens the report's credibility.` : '';
  ...
  5. "executive_summary" — a 2-3 paragraph non-technical overview written for the homeowner. State: (a) the concluded market value, (b) the current assessed value, (c) the overassessment in dollars and percentage, and (d) the 2-3 strongest arguments that support a reduction. Write in plain English — this is what the homeowner reads first. Close with: "This report documents [N] lines of evidence supporting a reduction. We recommend [action]."
  6. "property_description" — physical description emphasizing any characteristics that would LOWER value (age, deferred maintenance, functional obsolescence, layout inefficiencies)${hasPhotos ? '. Weave in specific observations from the photo evidence — reference individual photos by type (e.g., "as shown in the front elevation photograph") when describing condition.' : ''}
  7. "site_description_narrative" — site description noting any adverse factors (flood zone, noise, traffic, easements, irregular lot shape, proximity to commercial/industrial)${hasPhotos ? '. Reference any site-level issues visible in exterior photos (drainage, grading, neighboring conditions).' : ''}
  8. "improvement_description_narrative" — improvement description, emphasizing depreciation, outdated systems, and any features that don't add proportional value${hasPhotos ? '. Cite specific photo evidence of deterioration, deferred maintenance, and age-related wear documented in the condition analysis.' : ''}
  ${hasPhotos ? `9. "condition_assessment" — THIS IS YOUR MOST POWERFUL SECTION. The assessor valued this property without setting foot inside or examining it up close. The homeowner's photographs tell the REAL story. For each photo submitted:
  ...
  ${hasPhotos ? '10' : '9'}. "area_analysis_county" — ${county} County market context, noting any factors that suppress values (tax burden, population trends, economic conditions, inventory levels)
  ${hasPhotos ? '11' : '10'}. "area_analysis_city" — city-level analysis with focus on factors that limit appreciation
  ${hasPhotos ? '12' : '11'}. "area_analysis_neighborhood" — hyperlocal neighborhood analysis — competing listings, days on market, any negative externalities
  ${hasPhotos ? '13' : '12'}. "market_analysis" — market conditions and trends, emphasizing any softness, rising inventory, or declining price trends
  ${hasPhotos ? '14' : '13'}. "hbu_as_vacant" — highest and best use as if vacant, with legal, physical, financial, and productivity tests (verbatim definitions)
  ${hasPhotos ? '15' : '14'}. "hbu_as_improved" — highest and best use as improved, with legal, physical, financial, and productivity tests (verbatim definitions)
  ${hasPhotos ? '16' : '15'}. "sales_comparison_narrative" — aggressive comparable sales analysis: explain why each comp supports a LOWER value than the assessor's, call out every adjustment that works in the homeowner's favor. Use a detailed adjustment grid and summarize per-unit and total value. Show all adjustment factors and net adjustments. Present as a table and narrative.
  ${hasPhotos ? '17' : '16'}. "adjustment_grid_narrative" — methodical explanation of every adjustment, framed to support the lower concluded value. The condition adjustment line item should reference the photo evidence summary.
  ${payload.comparableRentals?.length ? `${hasPhotos ? '18' : '17'}. "income_approach_narrative" — rental income analysis showing the income-derived value is below the assessed value. Present rental comp table, pro forma, cap rate derivation (market and survey), and loaded cap rate calculation. Show all math step by step.` : ''}
  ${payload.overvaluationAnalysis?.costApproachValue != null ? `${hasPhotos ? '19' : '18'}. "cost_approach_narrative" — USPAP Cost Approach: present the replacement cost new (RCN), physical depreciation, ${payload.overvaluationAnalysis.functionalObsolescencePct ? 'functional obsolescence, ' : ''}and land value. Show the math step by step: "RCN of $[X] × (1 − [Y]% total depreciation) + land value of $[Z] = cost approach indicator of $[W]." If the cost approach value is BELOW the assessed value, this is a powerful third line of evidence converging with the sales comparison${payload.comparableRentals?.length ? ' and income approaches' : ''}. State explicitly: "Three independent valuation approaches all indicate a market value below the assessor's figure." If it exceeds the assessed value, address it honestly — explain why the cost approach may be less reliable here (e.g., land value uncertainty, market obsolescence) — do not suppress it.` : ''}
  ${payload.assessmentEquity?.isOverAssessed ? `${hasPhotos ? '20' : '19'}. "assessment_equity" — HORIZONTAL UNIFORMITY / EQUITY ARGUMENT — this section is MANDATORY when equity data shows over-assessment. Structure it as follows:
  ...
  ${hasPhotos ? '21' : '20'}. "reconciliation_narrative" — final value reconciliation: state the concluded value with conviction, quantify the exact overassessment in dollars and percentage, and recommend the assessment be reduced. When cost approach data is present, state which approaches were used and how they were weighted. Explicitly state that the concluded value reflects documented property condition from firsthand photographic evidence — evidence the assessor did not have when setting the assessed value.
  ${hasPhotos ? '22' : '21'}. "certification_and_limiting_conditions" — full USPAP certification and limiting conditions, verbatim. Use the language from the Appraisal Institute and the J2C report. This section must be complete and professional.
  ${hasPhotos ? '23' : '22'}. "appeal_argument_summary" — REQUIRED for tax_appeal reports. Write a numbered list of the 5 strongest appeal arguments, each as a clear, quotable statement the homeowner can read aloud to the board. Lead with the #1 strongest argument (equity data if available, otherwise comp-based overvaluation). End each argument with the dollar impact. Format: "1. [Argument headline]: [1-2 sentence explanation with specific numbers.] Dollar impact: $[X]." After the 5 arguments, add a one-sentence closing statement template the homeowner can use at their hearing.
