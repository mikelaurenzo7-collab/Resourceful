...
export default function ExecutiveSummary({ data }: { data: ReportTemplateData }) {
  ...
  const supportedMethods = [
    profile.hasSalesApproach
      ? `sales comparison evidence from ${data.comparableSales.length} documented transaction${data.comparableSales.length === 1 ? '' : 's'}`
      : null,
    profile.hasIncomeApproach ? 'release-ready income capitalization evidence' : null,
    profile.hasCostApproach ? 'release-ready cost approach evidence' : null,
    assignmentKind === 'tax_appeal' && property.assessment_ratio != null
      ? 'jurisdiction assessment-level and equity evidence'
      : null,
  ...
  return (
    <View>
      <SectionHeader number="I-B" title="Executive Valuation Summary" />
      ...
      <Text style={[theme.bodyText, { marginTop: 8 }]}>
        ...
        {' '}An omitted approach is not treated as negative evidence. No physical inspection,
        licensed-appraiser certification, IAAO-conformance opinion, USPAP-compliance opinion, legal
        opinion, or third-party acceptance is implied unless the report expressly documents that work.
      </Text>
    </View>
  );
}
