...
export async function runPhotoAnalysis(
  ...
  const uploadedTypes = new Set<string>(photos.map((p) => p.photo_type).filter((t): t is NonNullable<typeof t> => t != null));
  const required = REQUIRED_PHOTO_TYPES[propertyType] ?? REQUIRED_PHOTO_TYPES.residential;
  const hasAllRequiredTypes = required.every((t) => uploadedTypes.has(t));
