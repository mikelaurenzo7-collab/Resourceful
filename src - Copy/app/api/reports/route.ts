...
import { generateReportAccessToken } from '@/lib/utils/report-access';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Dev exceptions:
    // 1) X-E2E-Service-Key matches service role key.
    // 2) Localhost-only dev runs can bypass auth for founder emails.
    let founderAccess = false;
    if (isFounderEmail(client_email)) {
      const e2eKey  = request.headers.get('x-e2e-service-key');
      const svcKey  = process.env.SUPABASE_SERVICE_ROLE_KEY;
      const isE2E   = process.env.NODE_ENV !== 'production' && !!e2eKey && !!svcKey && e2eKey === svcKey;
      const isLocalhostDev = process.env.NODE_ENV !== 'production'
        && (request.nextUrl.hostname === 'localhost' || request.nextUrl.hostname === '127.0.0.1');

      if (isE2E) {
        founderAccess = true;
      } else if (isLocalhostDev) {
        founderAccess = true;
      } else {
    ...
    if (founderAccess) {
      ...
      const reportAccessToken = generateReportAccessToken(report.id);
