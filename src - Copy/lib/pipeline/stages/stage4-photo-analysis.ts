...
export async function runPhotoAnalysis(
  ...
  let claudeVisionAggregate: string | null = null;
  try {
    // Collect deferred-maintenance photos + any photos where Claude found significant defects.
    ...
    const deferredPhotos = photos.filter((p) => {
      ...
      // If Claude found significant defects anywhere, also include exterior photos
      // to give the aggregate analysis a fuller picture of the property's condition
    ...
    if (deferredPhotos.length > 0) {
      ...
      if (base64Images.length > 0) {
        ...
        pipelineLogger.info(
          ...
          '[stage4] Sending deferred maintenance photos to Claude Vision [multi-image aggregate + extended thinking]'
        );
        ...
        if (geminiResult) {
          ...
          const defectStr = geminiResult.primaryDefectType
            ...
            : '';
          claudeVisionAggregate = [
            `DEFERRED MAINTENANCE ANALYSIS [Claude Vision — ${base64Images.length} photos, extended thinking]:`,
          ...
          pipelineLogger.info(
            ...
            '[stage4] Claude Vision aggregate deferred maintenance complete'
          );
        }
      }
    }
  } catch (geminiError) {
    // Non-fatal — Claude Vision aggregate enriches but isn't required
    pipelineLogger.warn(
      { err: geminiError instanceof Error ? geminiError.message : String(geminiError) },
      '[stage4] Claude Vision aggregate analysis failed (non-fatal, continuing with individual photo results)'
    );
  }
  ...
  if (propertyDataForAge?.year_built) {
    ...
    const { error: ageUpdateError } = await supabase
      ...
      .update({
        ...
        ...(claudeVisionAggregate ? { condition_notes: claudeVisionAggregate } : {}),
      })
      .eq('report_id', reportId);
  ...
  } else if (claudeVisionAggregate) {
    // No year_built to compute effective age, but still store Claude Vision aggregate
    await supabase
      .from('property_data')
      .update({
        overall_condition: overallCondition,
        condition_notes: claudeVisionAggregate,
      })
      .eq('report_id', reportId);
  }
